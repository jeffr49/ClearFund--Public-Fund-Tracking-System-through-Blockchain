...
export default function ApproverHistoryPage() {
  ...
  useEffect(() => {
    ...
    async function fetchHistory() {
      try {
        const res = await fetch(`${API_BASE}/signer/history?wallet=${encodeURIComponent(user.wallet_address)}`);
      ...
      <div className="container" style={{ padding: "2rem", maxWidth: "1200px" }}>
        <header className="page-header" style={{ marginBottom: "2.5rem" }}>
          <h1 style={{ fontSize: "2.25rem", fontWeight: "800", color: "var(--text-primary)" }}>
            Approval History (Completed Projects)
          </h1>
          <p style={{ color: "var(--text-secondary)", fontSize: "1.1rem" }}>
            A historical log of your approvals and rejections for projects that have been completed.
          </p>
        </header>
        ...
        {loading ? (
          <div className="empty-state" style={{ padding: "4rem", textAlign: "center" }}>
            ...
            <h3 style={{ fontSize: "1.5rem" }}>Loading history...</h3>
          </div>
        ) : error ? (
          ...
          <div className="empty-state" style={{ padding: "5rem", textAlign: "center", background: "var(--card-bg)", borderRadius: "20px", border: "1px dashed var(--border-color)" }}>
            ...
            <h3 style={{ fontSize: "1.5rem", color: "var(--text-secondary)" }}>Empty History</h3>
            <p>You have no decisions recorded for completed projects yet.</p>
          </div>
        ) : (
